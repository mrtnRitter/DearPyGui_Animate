...
Features:
	- animates position, size and opacity
	- animations are bezier driven for smooth easing transistions
	- animations can be stacked to another, e.g. (left -> right) + (up -> down) = left up -> right down
	- animations can be looped
	- optional callbacks at animation end

	

Usage:
	- use 	dearpygui_animate.add(...)		to register an animation, see details below
	- set 	dearpygui_animate.run()			as render callback
	
	- call	dearpygui_animate.remove(...)	to terminate an animation midrun, see details below
	- call	dearpygui_animate.get(...)		to get animation values, see details below

	Details:
		- dearpygui_animate.add(...)
			type: 			string, one of "position", "size", "opacity"
			object: 		string, objectname to receive animation
			startval: 		list[int, int] or int, float
			endval: 		list[int, int] or int, float
			ease:			list[float, float, float, float], see: https://cubic-bezier.com/
			duration:		int, animation duration in frames
			
			-- optional --
			name:					string, name of animation, needed for .remove(...)
			timeoffset:				float or int, delay animation
			loop:					string, one of "ping-pong" (back to start), "cycle" (repeat from startval), "continue" (repeat from endval)
			callback:				callable, function to call when animation is finished, runs in same frame
			callback_data:			any, arguments for callback function
			early_callback: 		callable, function to call when animation is starting, runs in same frame
			early_callback_data: 	any, arguments for early callback function
			
			Example:
				dearpygui_animate.add("position", "MainWindow", [0,0], [500,150], [.2,0,.96,1], 60, name="WindowMove1", timeoffset=2, callback=fileDialog, callback_data="dir")
				dearpygui_animate.add("size", "MainWindow", [0,0], [200,75], [.85,0,.12,1], 30, name="WindowSize1")
				dearpygui_animate.add("opacity", "MainWindow", 0.4, 1, [.01,.45,.85,.19], 300, name="WindowFadeIn", timeoffset=0.0833)



		- dearpygui_animate.remove(...)
			name:			string, name of animation, must exist
			
			Example:
				dearpygui_animate.remove("WindowMove1")
			
			
			
		- dearpygui_animate.get(...)	
			string, anyone of, in any order:
				"name"
				"type"
				"object"
				"startval"
			...
			Example:
				dearpygui_animate.get("counter", "callback", "name"))
		
		
		
Known limitations:

	- position animation will work for windows only
		no method to set position of other items
	
	- minimum size of window is 32x32
		dearpygui_animate will handle smaller values, though windows cannot be smaller
		
	- minimum size of items is 1x1 (not tested for all items!)
		dearpygui_animate will handle smaller values, though items cannot be smaller



Future:
	- add methods for other setable properties
	- add generators for sin, cos, ramp, random, wiggle, ...
"""
